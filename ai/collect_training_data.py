...
class CollectTrainingData(object):
    def __init__(self, host, port, serial_port, input_size):
        ...
        self.connection = self.socket_server.accept()[0].makefile("rb")
    ...
    def collect(self):
        ...
        # stream video frames by one by one
        try:
            stream_bytes = b' '
            frame = 1
            ...
            try:
                np.savez_compressed(
                    directory + "/" + file_name + ".npz", train=X, train_labels=y)
            except IOError as e:
                print(e)
