...
def batch_generator(data_dir, image_paths, steering_angles, batch_size, is_training):
    ...
    # print(len(images), len(steers),image_paths.shape[0],np.random.permutation(image_paths.shape[0]))
    try:
        i = 0
        for index in np.random.permutation(image_paths.shape[0]):
            ...
            train_X[i] = preprocess(image)
            train_Y[i] = steering_angle
            i += 1
                # if i == batch_size:
                #     break
                # yield images, steers
