...
def find_lane_lines(img, prev_mid_points):
    ...
    for row in reversed(range(img.shape[0])):
        ...
        # the below row's lane lines.
            ...
            if row + 1 in right_lane_mid_points and row + 1 in left_lane_mid_points:
                ...
                start_search_point = int((right_lane_mid_points[row + 1] +
                                          left_lane_mid_points[row + 1]) / 2)

            # If row below only has one line, use offset_from_known_lane_line
            elif row + 1 in right_lane_mid_points:
                start_search_point = int(right_lane_mid_points[row + 1] -
                                         offset_from_known_lane_line)
            elif row + 1 in left_lane_mid_points:
                start_search_point = int(left_lane_mid_points[row + 1] +
                                         offset_from_known_lane_line)
        ...
        # If start_search_point is currently on a lane marking
        on_lane = img[row][start_search_point].all(
        )  # If it is currently on a lane marking
        if on_lane:
            # Put start_search_point as left side of right lane
        ...
        # right_lane_mid_points.
        if not right_lane_ended and right_lane_sides[
                0] != None and right_lane_sides[1] != None:
            right_lane_mid_points[row] = (right_lane_sides[0] +
                                          right_lane_sides[1]) / 2
            img[row][int(
                (right_lane_sides[0] + right_lane_sides[1]) / 2)] = (255, 0, 0)
        ...
        # left_lane_mid_points.
        if not left_lane_ended and left_lane_sides[
                0] != None and left_lane_sides[1] != None:
            left_lane_mid_points[row] = (left_lane_sides[0] +
                                         left_lane_sides[1]) / 2
            img[row][int(
                (left_lane_sides[0] + left_lane_sides[1]) / 2)] = (255, 0, 0)
    ...
    if len(right_lane_mid_points.keys()) != 0:
        ...
        if first_known_row - 5 in right_lane_mid_points:
            # Find slope
            slope = (right_lane_mid_points[first_known_row] -
                     right_lane_mid_points[first_known_row - 5]) / 5
    ...
    if len(left_lane_mid_points.keys()) != 0:
        ...
        if first_known_row - 5 in left_lane_mid_points:
            # Find slope
            slope = (left_lane_mid_points[first_known_row] -
                     left_lane_mid_points[first_known_row - 5]) / 5
...
def find_mid_points(img, right_lane_mid_points, left_lane_mid_points,
                    right_best_fit_function, left_best_fit_function):
    """
	Description:
    ...
    for row in range(img.shape[0]):
        ...
        if row > max(right_lane_mid_points, key=int) or row > max(
                left_lane_mid_points, key=int):
            ...
            # Find slope
            slope = (mid_points[last_known_row] -
                     mid_points[last_known_row - 20]) / 20

            # From last known row down to bottom if image, add in extrapodated
            # mid points.
            for row in range(last_known_row, img.shape[0]):
                col = (row -
                       last_known_row) * slope + mid_points[last_known_row]
                if col >= img.shape[1] or col < 0:
                    break
                mid_points[row] = col
                img[row][int(col)] = (255, 0, 0)
...
def find_center_offset(img, mid_points_best_fit_function,
                       lookahead_distance_pixels):
    center_offset = (mid_points_best_fit_function(lookahead_distance_pixels) /
                     img.shape[1]) - 0.5
...
def process_frame():
    ...
    ### Img Manipulation ###
    img = cv.resize(img, (int(orig_img.shape[1] * scale_down_factor),
                          int(orig_img.shape[0] * scale_down_factor)))
    img = cv.bitwise_not(img)
    img = cv.blur(img, (3, 3))
    img = cv.adaptiveThreshold(img, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C,
                               cv.THRESH_BINARY, 7, -10)
    ...
    ### Find lane lines ###
    right_lane_mid_points, left_lane_mid_points = find_lane_lines(
        img, prev_mid_points)
    ...
    mid_points = find_mid_points(img, right_lane_mid_points,
                                 left_lane_mid_points, right_best_fit_function,
                                 left_best_fit_function)
    ...
    turn_value, mid_points_best_fit_function = find_turn_value(
        mid_points, lookahead_distance_pixels)

    ### Calculate offset from center of lane ###
    center_offset = find_center_offset(img, mid_points_best_fit_function,
                                       lookahead_distance_pixels)
